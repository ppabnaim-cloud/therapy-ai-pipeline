...
st.set_page_config(page_title="TherapyAI — HTPN", page_icon="🧠", layout="centered")
...
input_method = st.radio("Choose input method:",
    ["📁 Upload Audio File", "⌨️ Paste Transcript"], horizontal=True)
...
if input_method == "📁 Upload Audio File":
    ...
    transcript_text = st.text_area("Paste transcript here", height=200,
        placeholder="Therapist: How have you been?\nPatient: ...")
...
if run_btn and transcript_text:
    ...
    analysis = parse_json(run_claude("""Analyse this therapy session. Return ONLY valid JSON:
...
ANALYSIS: """ + json.dumps(analysis), max_tokens=1000))
    ...
    audit = parse_json(run_claude("""Medicolegal documentation audit. Return ONLY valid JSON:
{"consent_documented":false,"risk_assessment_complete":false,
"safeguarding_addressed":false,"management_plan_present":false,
"follow_up_documented":false,"flags":[],"overall_adequacy":"",
...
NOTE: """ + note_text[:1000], max_tokens=1000))
    ...
    st.success("✅ Complete — review all outputs below before filing")

elif run_btn and not transcript_text:
    st.error("Please upload audio or paste a transcript first.")
...
if st.session_state.results:
    ...
    tab1,tab2,tab3,tab4,tab5 = st.tabs([
        "📋 Analysis","📄 Note","🏷️ Codes","💡 Recs","⚖️ Legal"])
...
with st.form("feedback_form"):
    ...
    fb_role  = st.selectbox("Role", ["Psychiatrist","Clinical Psychologist",
        "Counsellor","Medical Officer","Nurse","Other"])
    ...
    if st.form_submit_button("📨 Submit Feedback", use_container_width=True):
        ...
        entry = {"timestamp": datetime.now().strftime("%Y-%m-%d %H:%M"),
            "name":fb_name,"role":fb_role,"validity":fb_validity,
            "feasibility":fb_feasibility,"useful":fb_useful,
            "improvements":fb_improve,"comments":fb_comments}
