...
_ph_visitors = _col1.empty()
_ph_analyses = _col2.empty()
_ph_date     = _col3.empty()

def refresh_metrics():
    live = get_counts()
    _ph_visitors.metric("👥 Total Visitors",  live.get("visitors", 0))
    _ph_analyses.metric("🤖 AI Analyses Run", live.get("analyses", 0))
    _ph_date.metric("📅 Today",               datetime.now().strftime("%d %b %Y"))
            ...
            with st.spinner("Extracting clinical information..."):
                ...
                response   = client.messages.create(
                    ...
                    messages=[{"role":"user","content":[
                        ...
                        {"type":"text","text":"Extract all clinical information. Structure as: DOCUMENT TYPE: PATIENT DETAILS: REFERRING CLINICIAN: REASON FOR REFERRAL: RELEVANT HISTORY: CURRENT MEDICATIONS: INVESTIGATIONS: CLINICAL FINDINGS: RECOMMENDATIONS: Write Not stated for absent sections."}
                    ]}])
...
if run_btn and final_transcript:
    ...
    analysis = parse_json(run_claude("""Analyse this therapy session. Transcript may be in English or Bahasa Malaysia. Generate analysis in English. Return ONLY valid JSON: {"presenting_themes":[],"cognitive_distortions":[],"therapeutic_alliance":0,"patient_engagement":0,"disclosure_depth":"","resistance_indicators":[],"therapist_technique":[],"rupture_repair":false,"risk_flags":[],"session_quality_score":0,"key_patient_quotes":[],"summary":""}
""" + combined_context))
...
ANALYSIS: {json.dumps(analysis)}""")
    ...
    codes = parse_json(run_claude("""Suggest diagnostic codes. Return ONLY valid JSON: {"icd11_primary":{"code":"","description":""},"icd11_secondary":[],"dsm5_primary":{"code":"","description":"","criteria_met":[]},"dsm5_differential":[],"coding_confidence":"","coding_notes":"","suggested_assessments":[]}
ANALYSIS: """ + json.dumps(analysis), max_tokens=1000))

    progress.progress(70, text="Stage 4 of 5 — Recommendations...")
    rec = parse_json(run_claude("""Clinical supervision recommendations. Return ONLY valid JSON: {"guided_questions_next_session":[],"therapy_technique_suggestions":[],"therapy_modality_adjustment":"","homework_suggestions":[],"escalation_required":false,"escalation_reason":null,"supervisor_review_recommended":false,"overall_recommendation":""}
ANALYSIS: """ + json.dumps(analysis), max_tokens=1000))

    progress.progress(90, text="Stage 5 of 5 — Documentation quality check...")
    audit = parse_json(run_claude("""Review clinical note for documentation completeness. Return ONLY valid JSON: {"consent_documented":false,"risk_assessment_complete":false,"safeguarding_addressed":false,"management_plan_present":false,"follow_up_documented":false,"missing_elements":[],"documentation_quality_score":0,"overall_completeness":"","standard_recommendations":[],"priority_actions":[]}
NOTE: """ + note_text[:1000], max_tokens=1000))

    progress.progress(100, text="✅ Complete")
    st.session_state.results = {"analysis":analysis,"note_text":note_text,"codes":codes,"rec":rec,"audit":audit,"date_str":date_str}
    refresh_metrics()
...
if st.session_state.results:
    ...
    st.divider()
    tab1,tab2,tab3,tab4,tab5 = st.tabs(["📋 Analysis","📄 Clinical Note","🏷️ ICD-11 / DSM-5","💡 Recommendations","📝 Documentation Quality"])
    ...
    with tab2:
        ...
        with dl1:
            st.download_button(label="⬇️ Download .txt (Notepad)", data=edited_note.encode("utf-8"),
                file_name=f"{patient_id}_{date_str}_session{session_num}_note.txt", mime="text/plain", use_container_width=True)
        with dl2:
            ...
            st.download_button(label="⬇️ Download .docx (Word)", data=buf.read(),
                file_name=f"{patient_id}_{date_str}_session{session_num}_note.docx",
    ...
    with tab5:
        st.info("**Documentation Quality & Completeness Checker** — Reviews note against standard requirements.")
        ...
        try:
            resp = _req.post("https://api.emailjs.com/api/v1.0/email/send",
                json={"service_id":"service_z0rh03y","template_id":"template_6bzqjul","user_id":"jRs1TWPZym8iUjNck",
                    ...
                    "improvements":fb_improve or "None","comments":fb_comments or "None","timestamp":timestamp}},
                headers={"Content-Type":"application/json"}, timeout=10)
            if resp.status_code == 200:
                st.success("✅ Thank you. Feedback sent to Dr Naim (ppnaim@moh.gov.my)")
            else:
                st.warning(f"Email failed: {resp.text}")
        except Exception as e:
            st.warning(f"Email failed: {str(e)}")

st.divider()
st.markdown("""<div style='text-align:center;color:#888;font-size:0.75em;padding:8px;'>
🧠 <strong>TherapyAI — Demo Version</strong> · Dr Naim AI Team · Hospital Tengku Permaisuri Norashikin (HTPN) · 2026<br>
<em>Clinical automation support only. No patient records stored. All outputs require clinician verification before use.</em>
